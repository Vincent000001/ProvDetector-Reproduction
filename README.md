...
1) 现在复现的是**论文方法在 Detection24 数据上的可重复结果**，并非“论文原始数据集上的逐点复现”。
2) 指标非常高（甚至有 seed 接近 1.0），这可能来自 **Detection24 任务本身可分性较强**；故需要做：
   - leak-free；
   - 多 seed 报告均值±方差；
   - 给出复现脚本与固定随机性。
...
> 为什么需要 benign 切分扩充？
> Detection24 的 benign 原始文件数较少（最初只有 12 个 benign CSV），
